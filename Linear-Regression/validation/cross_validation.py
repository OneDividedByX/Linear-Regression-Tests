...
from miscelaneous.miscelaneous import *
...
def PrintCrossValidationStats(X,Y,f,l,index_vector,index_vector_complement,i,Inf,Sup,smooth,dec):
    # inicio = time.time()
    ...
    print('-------------------------------------------------------')
    print(f'El estimador Non-Pen (MSE{i+1}={round(MSE(Xi,Yi,f,0),dec)}) es:')
    theta_i=Estimator(Xi,Yi,f,0)
    for j in range(len(theta_i)):
        theta_i[j]=float(round(theta_i[j],dec))    
    theta_i=pd.DataFrame({'estimator':theta_i})
    print(theta_i.T)
    # PrintVectorH(theta_i)
    print('-------------------------------------------------------')
    print(f'El estimador Ridge (l={l}) (MSE{i+1}={round(MSE(Xi,Yi,f,l),dec)}) es:')
    theta_i_lambda=Estimator(Xi,Yi,f,l)
    for j in range(len(theta_i_lambda)):
        theta_i_lambda[j]=float(round(theta_i_lambda[j],dec))
    theta_i_lambda=pd.DataFrame({'estimator':theta_i_lambda})
    print(theta_i.T)
    # PrintVectorH(theta_i_lambda)
    print('-------------------------------------------------------')
    print(f'El estimador de validación simple Ridge (VAL{i+1}) es: {round(SimpleValidation(X,Y,f,l,index_vector),dec)}')
    print('-------------------------------------------------------')
...
def GraphCrossValVSPenalty(X,Y,f,l_min,l_max,l_step,S):
    ...
    # while j<l_axis_len: # A kind of shuffle operation for K-folds
    #   l_axis[j]=l
    #   CV_Estimator=0
    #   for i in range(0,S):
    ...
    while j<l_axis_len:
      l_axis[j]=l
      theta_MCO,theta_Ridge,CV_Estimator=EstimatorsMeanKFoldsRidgeMCO(X,Y,f,S,l)          
      CV_axis[j]=CV_Estimator; j=j+1
      l=l+l_step
    # plt.plot(l_axis, CV_axis,'o',color='green')
    # PrintVectorH(l_axis)
    # PrintVectorH(CV_axis)
    plt.plot(l_axis, CV_axis,color='green',label= '$(l,CV(l))$')
    plt.legend(loc="upper left")
    # plt.gca().set_aspect('equal')
    plt.grid()
    plt.show()
...
def GraphKFoldsRidgeMCO(X,Y,f,K_fold,l,Inf,Sup,smooth,dec):
  print('')
  print('-------------------------------------------------------')
  ...
  for i in range(0,K_fold): #K_fold-fold
    ...
    index_vector_complement=GetVectorComplement(X,index_vector)
    index_vector,index_vector_complement=SwapVectors(index_vector,index_vector_complement)
    PrintCrossValidationStats(X,Y,f,l,index_vector,index_vector_complement,i,Inf,Sup,smooth,dec) # Inf and sup are the limits on AxisX
    CV=SimpleValidation(X,Y,f,l,index_vector)
    CV_Estimator+=CV
    print(''); print('')
  ...
  if l>0:
    print('-------------------------------------------------------')
    print(f'El estimador de Validacion Cruzada Ridge (l={l},K={K_fold}) es: {round(CV_Estimator,dec)}')
    print('-------------------------------------------------------')
  else:
    print('-------------------------------------------------------')
    print(f'El estimador de Validacion Cruzada MCO (K={K_fold}) es: {round(CV_Estimator,dec)}')
    print('-------------------------------------------------------')
...
def MeanSummaryKFoldsRidgeMCO(X,Y,f,K_fold,l,dec):
    theta_MCO_mean,theta_Ridge_mean,CV_Ridge=EstimatorsMeanKFoldsRidgeMCO(X,Y,f,K_fold,l)
    print(f'Estimador MCO promedio (K={K_fold}):')
    ...
    print(v.T)
    print('')
    print(f'Estimador Ridge (l={l}) promedio (K={K_fold}):')
    ...
    print(v.T)
    print('-------------------------------------------------------')
    print(f'Estimador de Validacion Cruzada Ridge (l={l},K={K_fold}): {round(CV_Ridge,dec)}''')
